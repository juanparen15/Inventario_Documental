...
@section('title', 'Codigo de Dependencia')
@section('style')
    <!-- SweetAlert2 -->
                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">Codigo de Dependencia</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
            ...
            <div class="container-fluid">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Codigo de Dependencia</h3>
                        <div class="card-tools">

                            <a href="{{ route('admin.proyectos.create') }}" class="btn btn-primary">
                                Agregar Codigo de Dependencia
                            </a>
                        </div>
                    </div>
                    <!-- /.card-header -->
                            ...
                            <tbody>
                                ...
                                @foreach ($requiproyectos as $requiproyecto)
                                    <tr>
                                        ...
                                        <td>{{ $requiproyecto->area->nomarea }}</td>
                                        <td>{{ $requiproyecto->detproyecto }}</td>
                                        <td>

                                            <form action="{{ route('admin.proyectos.destroy', $requiproyecto) }}"
                                                method="POST">
                                                @csrf
                                                @method('delete')

                                                <a class="btn btn-primary btn-sm"
                                                    href="{{ route('admin.proyectos.edit', $requiproyecto) }}">Editar</a>
...
@section('script')
    ...
    @if (session('flash') == 'actualizado')
        <script>
            $(function() {
                ...
                Toast.fire({
                    icon: 'success',
                    title: 'El Codigo de Dependencia se Actualizó con Exito.'
                })
            });
        </script>
    @endif
    @if (session('flash') == 'registrado')
        <script>
            $(function() {
                ...
                Toast.fire({
                    icon: 'success',
                    title: 'El Codigo de Dependencia se Creó con Exito.'
                })
            });
        </script>
    @endif
    @if (session('flash') == 'eliminado')
        <script>
            Swal.fire(
                '¡Eliminado!',
                'El Codigo de Dependencia se Eliminó con Exito.',
                'success'
            )
        </script>
    @endif
